...
const SPLASH_CONFIG = {
    ...
    isMatrixMode: true // Special flag for analyze page
};
...
class SplashScreen {
    ...
    init() {
        ...
        // Setup Matrix canvas instead of video 
        if (SPLASH_CONFIG.isMatrixMode) {
    ...
    initMatrixEffect() {
        ...
        const draw = () => {
            // Only draw if matrix mode is active and element still exists
            if (!document.getElementById('analyzerCanvas')) return;

            ctx.fillStyle = 'rgba(10, 15, 30, 0.15)';
            ctx.fillRect(0, 0, canvas.width, canvas.height);

            ctx.fillStyle = '#10b981'; // emerald green to match theme
            ctx.font = fontSize + 'px "Plus Jakarta Sans", monospace';
            ctx.fontWeight = 'bold';

            for (let i = 0; i < drops.length; i++) {
                // Draw when drop is visible
                if (drops[i] > 0) {
                    const text = characters[Math.floor(Math.random() * characters.length)];

                    // Add a bright white head to the falling text
                    ctx.fillStyle = drops[i] * fontSize > canvas.height - 50 ? '#ffffff' : (Math.random() > 0.95 ? '#ffffff' : '#34d399');
                    ctx.fillText(text, i * fontSize, drops[i] * fontSize);
                }

                // Reset drop to top with slight randomization
